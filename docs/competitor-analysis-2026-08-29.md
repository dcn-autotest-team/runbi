...
> 以下为领域知识(官网 WAF 拦截或 JS 渲染无法实抓),已标注

| 产品 | 形态 | 对 Runbi 的威胁度 |
|---|---|---|
| 秘塔写作猫 | Web + 插件,中文改写/校对 | 中(场景重叠但非系统级) |
| WPS AI | Office 生态内润色 | 低-中(锁在 WPS 内) |
| 豆包浏览器插件 | 划词 AI 问答/总结(字节流量) | 高(C 端免费碾压) |
| Kimi 浏览器助手 | 划词 AI(月活用户巨大) | 高 |
| 沉浸式翻译 | 划词翻译+双语对照,口碑极好 | 中(翻译强,润色弱) |
| 搜狗/讯飞/百度输入法 AI | 输入框内 AI 润色(即时场景) | 高(但仅限输入法场景) |

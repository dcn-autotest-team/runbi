...
/**
 * 从 OpenAI 兼容端点 GET /models 拉取可用模型 id 列表。
 * API Key 可为空（本地服务）；HTTP 非 2xx 与网络错误都折叠进 error 字段。
...
export async function fetchCompatibleModels(
  ...
  try {
    const res = await fetchImpl(`${baseUrl.trim().replace(/\/+$/, '').replace(/\/chat\/completions$/, '')}/models`, {
    ...
    const models = [...new Set(parseNameList(await res.json(), 'data', 'id'))];
    return models.length ? { models } : { models: [], error: '端点未返回可用模型' };
  } catch (e) {
...
export async function fetchSenseAudioModels(
  fetchImpl?: typeof fetch, baseUrl = SENSEAUDIO_BASE_URL, apiKey?: string, timeoutMs = 8000
): Promise<{ models: string[]; error?: string }> {
  if (!apiKey?.trim()) return { models: [], error: '未配置 API Key；请先填写 API Key。' };
  return fetchCompatibleModels(fetchImpl, baseUrl, apiKey, timeoutMs);
}

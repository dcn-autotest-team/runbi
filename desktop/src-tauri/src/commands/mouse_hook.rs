...
unsafe extern "system" fn low_level_mouse_proc(
    ...
    if n_code >= 0 {
        ...
        // Remote-desktop and accessibility tools legitimately deliver injected
        // mouse events. Ignore only Runbi's own internal action window; a
        // blanket injected-event filter makes remote text selection incapable
        // of opening the capsule.
        let internal_mouse_action = MONITOR_STATE
            .get()
            .map(|(state, _)| state.is_internal_action.load(Ordering::Relaxed))
            .unwrap_or(false);
        if hook_struct.flags & LLMHF_INJECTED != 0 && internal_mouse_action {
